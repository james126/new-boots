...
const ProjectInformation = (props: ProjectInformationProps) => {
    ...
    useEffect(() => {
        if (displayError) {
            ...
            setError(prev => ({ ...prev, startDate: dateError }));
            
            // Check all errors including date after small delay
            setTimeout(() => {
                const allErrors = { ...error, startDate: dateError };
                if (!Object.values(allErrors).includes(true)) {
                    props.handleNextPage();
                }
            }, 50);
        }
    }, [displayError, error, date, props]);
    ...
    const nextPage = () => {
        ...
        // Set display error (this will trigger useEffect for date validation)
