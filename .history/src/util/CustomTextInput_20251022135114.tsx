...
const CustomInput = forwardRef<CustomInputRef, CustomInputProps>((props, ref) => {
  ...
  const validateInput = (input: string | number | null) => {
    ...
    if (input === null || input === undefined) {
        ...
        console.log(`Validating date input ${props.label} ${dateStr}: ${errorState ? 'INVALID' : 'VALID'}`);
    }
...
const getDisplayValue = () => {
  if (props.inputType === InputTypes.DATE) {
    // If internal state holds a Date, format it
    ...
    // If internal state is a string, try to parse it to a Date first
    if (typeof input === 'string' && input.length > 0) {
      ...
      // If parsing failed, assume it's already in desired display format
        value={getDisplayValue()}
    }
    // Fallback to props.value if available and parsable
