...
import { formCardStyle } from '../styles/templates/formCardStyle';
...
const CustomInput = forwardRef<CustomInputRef, CustomInputProps>((props, ref) => {
  ...
  const [hasError, setHasError] = useState<boolean>(false); // local error state
  ...
  const validateInput = (input: string | number | null) => {
    ...
    if (input === null || input === undefined) {
        ...
        errorState = true;
    } else if (props.inputType === InputTypes.DATE && props.value instanceof Date !== true) {
        errorState = true;
    }
  ...
  const handleTextChange = (inputText: string) => {
    // For DATE inputs, don't allow manual text changes - only allow picker
    if (props.inputType === InputTypes.DATE) {
        return; // Prevent manual editing of date fields
    }
  ...
  useImperativeHandle(ref, () => ({
    ...
    nextPage: () => {
      ...
      //console.log(`Updating ${props.label} input:`, input, 'Error:', errorState);
    }
  }), [input, props.updateParent]); // Add dependencies

  const getDisplayValue = () => {
    if (props.inputType === InputTypes.DATE && props.value instanceof Date) {
        return props.value.toLocaleDateString('en-GB', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric'
        });
  ...
  return (
    <View style={props.viewStyle}>
      <TextInput
        ...
        contentStyle={[theme.fonts.bodyMedium, {paddingLeft: 16}]}
      ...
      />
          <HelperText type="error" visible={hasError && props.error} padding="none" style={{ paddingTop: 0, paddingBottom: 0, paddingLeft: 16 }}>
